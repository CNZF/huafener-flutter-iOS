    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                if (tzImagePickerVc.allowPickingVideo == YES && tzImagePickerVc.allowPickingImage == NO) {
                    [tzImagePickerVc showAlertWithTitle:[NSString stringWithFormat:@"你最多只能选择%zd个视频",tzImagePickerVc.maxImagesCount]];
                } else {
...
//        index = indexPath.row - 1;
    ...
    if (_models.count == 0 || index > _models.count - 1) {
        return;
    }
    TZAssetModel *model = _models[index];
    if (model.type == TZAssetModelMediaTypeVideo) {
        if (tzImagePickerVc.selectedModels.count > 0 && tzImagePickerVc.allowPickingVideo == NO) {
            TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
