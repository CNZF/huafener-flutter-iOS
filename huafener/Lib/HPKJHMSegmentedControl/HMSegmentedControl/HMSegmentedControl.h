...
typedef void (^SelectedIndexTouchedBlock)(NSInteger index);
...
/**
 已经选中的tag，再次点击，会调用这个block
 */
@property (nonatomic, copy) SelectedIndexTouchedBlock selectedIndexTouchedBlock;
...
@property (nonatomic, assign) CGFloat segmentWidthFixed;

@property (nonatomic, assign) CGFloat segmentIndicatorLineWidth;
...
- (void)hideUnreadTip:(BOOL)showTip withIndex:(NSInteger)index;
