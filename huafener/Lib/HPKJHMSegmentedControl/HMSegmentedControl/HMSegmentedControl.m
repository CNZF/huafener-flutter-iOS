...
@property (nonatomic, strong) NSMutableArray *unreadTipArray;
@property (nonatomic, strong) NSMutableArray *titleLayerArray;
...
- (void)setupTipArray {
    self.unreadTipArray = [NSMutableArray arrayWithCapacity:self.sectionTitles.count];
    for (int i=0; i<self.sectionTitles.count; i++) {
        [self.unreadTipArray addObject:[NSNumber numberWithBool:YES]];
    }
}

- (NSMutableArray *)titleLayerArray {
    if (!_titleLayerArray) {
        _titleLayerArray = [NSMutableArray array];
    }
    return _titleLayerArray;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    [self.titleLayerArray removeAllObjects];
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            // 未读消息提醒
             ...
             layer.frame = CGRectMake(36, 0, 6, 6);
             layer.cornerRadius = 3;
            NSNumber *number = self.unreadTipArray[idx];
            layer.hidden = number.boolValue;

             [layer setNeedsDisplay];
            [titleLayer addSublayer:layer];
            [self.scrollView.layer addSublayer:titleLayer];
            [self.titleLayerArray addObject:titleLayer];
...
//            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2.0f);
            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight - stringHeight) / 2.0f);
...
//            CGRect textRect = CGRectMake(textXOffset, yOffset, textWidth, stringHeight);
            ...
            CGRect textRect = CGRectMake(textXOffset, imageHeight + 10, textWidth, stringHeight);
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.segmentIndicatorLineWidth) {
        //固定的线宽度
        sectionWidth =self.segmentIndicatorLineWidth;
    }else{
        if (self.type == HMSegmentedControlTypeText) {
...
- (void)updateSegmentsRects {
    ...
    if (self.segmentWidthFixed) {
        self.segmentWidth = self.segmentWidthFixed;
    }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        if (segment != self.selectedSegmentIndex && segment < sectionsCount) {
            ...
            // 该tag已经选中，再次点击
            if (self.isTouchEnabled && self.selectedIndexTouchedBlock) {                    self.selectedIndexTouchedBlock(self.selectedSegmentIndex);
            }
        }
    }
}
...
- (void)hideUnreadTip:(BOOL)hideShowTip withIndex:(NSInteger)index {
    for (int i=0; i<self.titleLayerArray.count; i++) {
        CATextLayer *textLayer = self.titleLayerArray[i];
        NSAttributedString *layerString = textLayer.string;
        if (i == index) {
            [self.unreadTipArray replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:hideShowTip]];
            for (CALayer *redLayer in textLayer.sublayers) {
                redLayer.hidden = hideShowTip;
            }
        }
    }
}
